    ...
    try:
        result = subprocess.run(
            [
                ...
                # workflow files reaching the filter is harmless, since
                # the is_file() check below drops paths that no longer
                # exist.
                "--diff-filter=ACDMR",
                f"{base_sha}..{head_sha}",
            ],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--config-path",
        default=os.environ.get("SCANNER_ZIZMOR_CONFIG_PATH", ""),
        help=(
            "Optional zizmor config path relative to --checkout-root. "
            "When omitted, zizmor's conventional locations are discovered "
