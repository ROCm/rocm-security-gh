...
"""Download and verify third-party scanner binaries.

Shared by every scanner script that downloads a release tarball at run
time (gitleaks.py, trivy.py, zizmor.py, ...): each resolves its own
tarball URL/filename and calls `download_and_verify_tarball()`, which
looks up the expected digest recorded in this repo's shared
`checksums.sha256` and refuses to extract or use the binary on a
mismatch.
"""
...
_DEFAULT_MAX_TARBALL_BYTES = 100 * 1024 * 1024  # 100 MiB guardrail
...
def expected_sha256(repo_root: Path, filename: str) -> str:
    ...
    block use of the binary, not silently skip verification.
    """
...
def download_and_verify_tarball(
    *,
    ...
    member_name: str,
    install_dir: Path,
    max_bytes: int = _DEFAULT_MAX_TARBALL_BYTES,
